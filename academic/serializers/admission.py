...
class PublicAdmissionApplicationSerializer(serializers.ModelSerializer):
    """Applicant-facing application state; deliberately excludes staff internals."""

    full_name = serializers.CharField(read_only=True)
    age = serializers.IntegerField(read_only=True)
    applying_for_class_name = serializers.SerializerMethodField()
    admission_session_name = serializers.CharField(
        source='admission_session.name', read_only=True
    )
    status_display = serializers.CharField(source='get_status_display', read_only=True)
    can_submit = serializers.BooleanField(read_only=True)
    can_accept_offer = serializers.BooleanField(read_only=True)
    ...
    class Meta:
        ...
        fields = [
            ...
            'admission_session', 'admission_session_name',
            'applying_for_class', 'applying_for_class_name',
            'first_name', 'middle_name', 'last_name', 'full_name', 'age',
            'gender', 'date_of_birth', 'state_of_origin', 'lga', 'religion',
            'blood_group', 'address', 'city', 'parent_first_name',
            'parent_last_name', 'parent_email', 'parent_phone',
            'parent_occupation', 'parent_relationship', 'alt_contact_name',
            'alt_contact_phone', 'alt_contact_relationship', 'previous_school',
            'previous_class', 'medical_conditions', 'special_needs',
            'application_fee_paid', 'exam_fee_paid', 'acceptance_fee_paid',
            'acceptance_deadline', 'submitted_at', 'accepted_at',
            'can_submit', 'can_accept_offer', 'created_at', 'updated_at',
        ]
        read_only_fields = fields

    def get_applying_for_class_name(self, obj):
        return str(obj.applying_for_class)
...
class AdmissionApplicationUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if 'status' in self.initial_data:
            raise serializers.ValidationError({
                'status': 'Lifecycle status must be changed through an explicit workflow action.'
            })
...
class PublicAdmissionDocumentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = AdmissionDocument
        fields = [
            'public_id', 'document_type', 'document_type_display', 'file',
            'file_url', 'description', 'uploaded_at',
        ]
        read_only_fields = ['public_id', 'document_type_display', 'file_url', 'uploaded_at']
...
class PublicAdmissionDocumentUploadSerializer(serializers.ModelSerializer):
    class Meta:
        model = AdmissionDocument
        fields = ['document_type', 'file', 'description']

    def validate(self, attrs):
        if 'application' in self.initial_data:
            raise serializers.ValidationError({
                'application': 'The application is determined by the tracking credential in the URL.'
            })
        return attrs
