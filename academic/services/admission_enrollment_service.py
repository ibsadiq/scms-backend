...
from academic.models import (
    AdmissionApplication, AdmissionStatus, AcademicYear, ClassRoom, Parent, Student,
)
...
class AdmissionEnrollmentService:
    @classmethod
    @transaction.atomic
    def enroll(cls, *, application, classroom, actor):
        application = AdmissionApplication.objects.select_for_update().get(pk=application.pk)
        if application.enrolled_student_id or application.status == AdmissionStatus.ENROLLED:
            raise ValidationError("This application has already been enrolled.")
        if application.status != AdmissionStatus.ACCEPTED:
            raise ValidationError("Only an accepted application can be enrolled.")
        ...
        EnrollmentService.enroll(
            ...
            notes=f"Converted from {application.application_number}",
        )
        application.enrolled_student = student
        application.status = AdmissionStatus.ENROLLED
        application.save(update_fields=("enrolled_student", "status", "enrolled_at", "updated_at"))
        transaction.on_commit(lambda: cls._issue_parent_invitation(application, parent, actor))
        return student

    @staticmethod
    def _issue_parent_invitation(application, parent, actor):
        try:
            ...
            logger.exception(
                "Failed to issue parent invitation for admission application %s",
                application.pk,
            )
