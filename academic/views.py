...
class BulkUploadStudentsProfileView(APIView):
    """
    API View to handle bulk uploading of Student profiles from an Excel file.
    """
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            columns = [
                "first_name", "last_name", "gender", "date_of_birth", "class_level", 
                ...
                try:
                    ...
                    if row_data.get("parent_contact"):
                        ...
                        parent, created = Parent.objects.get_or_create(
                            user__phone_number=contact if Parent.objects.filter(user__phone_number=contact).exists() else None,
                            defaults={"first_name": parent_name}
                        )
                        # We don't fully handle creating a User here to avoid complexity.
                        # Usually, parent has a user. If we just get_or_create the parent model...
                        # A better way is:
