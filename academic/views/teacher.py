...
from academic.serializers.schema_contracts import (
    ClassroomStudentResponseSerializer,
    TeacherClassesResponseSerializer,
    TeacherScheduleEntrySerializer,
)
...
class TeacherMyClassesView(APIView):
    ...
    @extend_schema(responses={200: TeacherClassesResponseSerializer})
...
class ClassroomStudentsView(APIView):
    ...
    @extend_schema(
        parameters=[
            ...
            OpenApiParameter("component_id", int, required=False),
        ],
        responses={200: ClassroomStudentResponseSerializer(many=True)},
    )
    ...
    def post(self, request):
        ...
        with transaction.atomic():
            for record in records:
                ...
                _, created = StudentAttendanceService.mark_manual(
                    student=student,
...
class TeacherMyScheduleView(APIView):
    ...
    @extend_schema(
        parameters=[OpenApiParameter("day", str, required=False)],
        responses={200: TeacherScheduleEntrySerializer(many=True)},
    )
    def get(self, request):
        # Get the teacher associated with the logged-in user
        try:
