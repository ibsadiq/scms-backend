...
from academic.permissions import CanAccessStudentPortal
...
class StudentAuthViewSet(viewsets.ViewSet):
    ...
    def register(self, request):
        ...
        submitted_phone = normalize_phone_number(data['phone_number'])
        stored_phone = normalize_phone_number(student.phone_number)
        if not stored_phone or submitted_phone != stored_phone:
            return Response(
                {'error': 'Phone number does not match the student record. Contact administration.'},
                status=status.HTTP_400_BAD_REQUEST
            )

        # Check if phone number is already in use
        if CustomUser.objects.filter(phone_number=submitted_phone).exists():
            return Response(
        ...
        student.user = user
        student.phone_number = submitted_phone
        student.save()
...
class StudentPortalViewSet(viewsets.ViewSet):
    ...
    """
    permission_classes = [CanAccessStudentPortal]
