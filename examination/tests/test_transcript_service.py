...
class TranscriptServiceTestCase(TestCase):
    ...
    def test_transcript_generation(self):
        # 100%
        AssessmentEntry.objects.create(student=self.enrollment, term=self.term1, subject=self.subject, component=self.ca_component, score=30, entered_by=self.teacher)
        AssessmentEntry.objects.create(student=self.enrollment, term=self.term1, subject=self.subject, component=self.exam_component, score=70, entered_by=self.teacher)
        ...
        annual_res.lifecycle_state = "PUBLISHED"
        annual_res.save()
