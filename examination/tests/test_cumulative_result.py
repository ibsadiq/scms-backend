...
class CumulativeResultServiceTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.enrollment_y2 = StudentClassEnrollment.objects.create(
            student=self.student, classroom=self.classroom, academic_year=self.academic_year_2
        )
    ...
    def test_cumulative_result_computation(self):
        ...
        AssessmentEntry.objects.create(student=self.enrollment_y1, subject=self.subject, component=self.ca_component, score=20, entered_by=self.teacher)
        AssessmentEntry.objects.create(student=self.enrollment_y1, subject=self.subject, component=self.exam_component, score=60, entered_by=self.teacher)
        ...
        AssessmentEntry.objects.create(student=self.enrollment_y2, subject=self.subject, component=self.ca_component2, score=25, entered_by=self.teacher)
        AssessmentEntry.objects.create(student=self.enrollment_y2, subject=self.subject, component=self.exam_component2, score=40, entered_by=self.teacher)
